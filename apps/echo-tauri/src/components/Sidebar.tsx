...
export type TabKey = "chat" | "mcp" | "skill" | "provider";
type NavKey = Exclude<TabKey, "chat"> | "settings";
...
export default function Sidebar({
  ...
  return (
    <aside className="flex w-80 shrink-0 flex-col border-r border-[#2a2a2a] bg-[#1c1f26]">
      ...
      <nav className="space-y-1 px-2.5 pb-2">
        {ITEMS.map((item) => {
          ...
          return (
            <button
              key={item.key}
              onClick={() => item.key !== "settings" && onChange(item.key)}
              className={
